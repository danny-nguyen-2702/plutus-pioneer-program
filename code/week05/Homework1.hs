...
mkPolicy pkh deadline () ctx = traceIfFalse "tx doesn't sign by the token's owner"  isSigned            &&
                               traceIfFalse "tx has passed the deadline"            notReachDeadline
    where
        txInfo :: TxInfo
        ...
        notReachDeadline :: Bool
        notReachDeadline = (to deadline) `contains` txInfoValidRange txInfo
