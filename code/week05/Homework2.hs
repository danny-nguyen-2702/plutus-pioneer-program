...
mkPolicy oref () ctx = traceIfFalse "the given UTXO is not consumed" isConsumedUTXO &&
  ...
  where 
    ...
    isConsumedUTXO :: Bool
    isConsumedUTXO = any (\txInInfo -> txInInfoOutRef txInInfo == oref) $ txInfoInputs txInfo

    onlyMintOne :: Bool
    onlyMintOne = case flattenValue (txInfoMint txInfo) of
      [(cs, tn', amount)] -> cs == ownCurrencySymbol ctx && tn' == tn && amount == 1
      _                   -> False
